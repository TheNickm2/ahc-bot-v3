...
export class AuctionLotHelper {
  ...
  constructor() {
    if (!process.env.DISCORD_AUCTION_SHEET_ID) {
      throw new Error(
        "Environment Configuration Error. Discord Auction Sheet ID is required",
      );
    }
    this.googleSpreadsheetUtil = new GoogleSpreadsheetUtil(
      process.env.DISCORD_AUCTION_SHEET_ID,
    );
  }
  ...
  private async loadAuctionLots() {
    ...
    rows.forEach((row) => {
      ...
      if (
        !rowData.Title ||
        !rowData.Description ||
        !rowData["Starting Bid"] ||
        Number.isNaN(rowData["Starting Bid"])
      )
        return;
