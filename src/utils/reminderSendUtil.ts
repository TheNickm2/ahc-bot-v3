...
import { ReminderMessageComponents } from './messageComponentUtil';
...
export async function sendReminder(reminder: ReminderRow): Promise<boolean> {
  ...
  try {
    ...
    await user.send({ components: [ReminderMessageComponents(reminder)], flags: [MessageFlags.IsComponentsV2] });
    return true;
  } catch (err) {
  ...
  // ! disabled to ensure reminders never spam important channels
  // try {
  //   const channel = await container.client.channels.fetch(reminder.channel_id);
  //   if (channel?.isSendable()) {
  //     await channel.send({ content: `<@${reminder.user_id}>`, components: [ReminderMessageComponents(reminder)], flags: [MessageFlags.IsComponentsV2] });
  //     return true;
  //   }
  // } catch (err) {
